    ...
    >
            ...
            {CATEGORIES.map((cat, i) => {
              ...
              return (
                <div key={cat} className="space-y-2">
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em]">{cat}</span>
                  {selectedAtThisLevel ? (
                    <motion.div 
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      className="bg-[#FFD700] text-black px-4 py-2 rounded-xl text-[11px] font-black flex justify-between items-center"
                    >
          ...
          {connections.map((conn) => {
            ...
            return (
              <motion.line 
                ...
                strokeWidth={from.isSelected && to.isSelected ? 2.5 : 1}
                strokeDasharray={from.isSelected && to.isSelected ? "none" : "8 5"}
            ...
            >
              <div className="relative flex flex-col items-center">
                <button
                  ...
                  `}
                >
                  <span className={`text-[16px] font-black leading-tight mb-2 ${node.isSelected ? 'text-black' : 'text-gray-900'}`}>
                    {node.label}
                  </span>
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-10 px-10 py-5 glass-panel bg-white/70 rounded-full border border-black/5 shadow-2xl z-30">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">左键: 聚焦/展开</span>
        </div>
        <div className="w-px h-5 bg-gray-200"></div>
        <div className="flex items-center gap-3">
          <div className="w-2.5 h-2.5 rounded-full bg-[#FFD700]"></div>
          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">右键: 层级单选</span>
        </div>
        <div className="w-px h-5 bg-gray-200"></div>
        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">点击空白: 恢复视图</div>
      </div>
    </motion.div>
  );
};
